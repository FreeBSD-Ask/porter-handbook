...
以下是移植过程中常见的一些注意事项和禁忌。请对照本清单检查 Port，同时也请查看其他人提交到 [PR 数据库](https://bugs.freebsd.org/search/) 中的 Ports。根据 [Bug Reports and General Commentary](https://docs.freebsd.org/en/articles/contributing/#CONTRIB-GENERAL) 中的说明提交对 Ports 的任何意见。检查 PR 数据库中的 Ports 不仅可以加快我们提交的速度，还可以证明你知道自己在做什么。
...
>**重要**
>
>要么使用 **bsd.port.pre.mk**/**bsd.port.post.mk** 配对，要么只使用 **bsd.port.mk**；请不要混用这两种方式。
...
| 变量名         | 描述                                                                                                             |
| ----------- | -------------------------------------------------------------------------------------------------------------- |
| `ARCH`      | 架构类型，`uname -p` 返回的结果（例如，`i386`）                                                                               |
...
Port 必须遵守 `CFLAGS`。这意味着不能直接覆盖该变量的值，而应在已有值的基础上追加所需内容。这样可以确保影响所有 Ports 的构建选项能被全局设置。
...
* `FORBIDDEN`：用于含有安全漏洞，或对 FreeBSD 系统安全构成严重隐患的 Port（例如：臭名昭著的不安全程序，或提供易被攻击的服务的程序）。一旦发现某个软件存在漏洞且无可用升级版本，应立即将相关 Port 标记为 `FORBIDDEN`。理想情况下，发现安全漏洞时应尽快升级 Port，以减少存在漏洞的 FreeBSD 主机数量（我们以安全著称），然而有时从漏洞披露到漏洞软件发布更新版本之间会存在明显的时间间隔。除安全问题外，请勿使用 `FORBIDDEN`。
...
FreeBSD 支持的处理器架构远不止常见的 x86 架构。一些 Port 具有特定于一个或多个架构的限制。
...
这类 Port 构建出的软件包，其架构字符串将以 `:*`（通配架构）结尾，而不是例如 `freebsd:13:x86:64`（amd64 架构）。
...
请记住，如果一个 Port 无法工作，`BROKEN` 和 `FORBIDDEN` 应仅作为临时手段使用。永久损坏的 Port 将被从 Ports 树中完全移除。
...
可以仅设置 `DEPRECATED` 而不设置 `EXPIRATION_DATE`（例如建议使用新版 Port），但反过来没有任何意义。

>**注意**
>
>标记 Port 为 `DEPRECATED` 时，若有替代 Port 可用，建议在提交说明中提及替代项。

并没有明确规定应提前多久通知用户。当前的惯例是：安全相关问题提前一个月通知，构建相关问题提前两个月通知。这也为感兴趣的提交者留出一点时间来修复问题。
...
除非用于目标（targets）中，否则不建议使用 **sysctl**。这是因为在求值任何 `makevar`（例如运行 `make index` 时所用到的）时会执行该命令，从而进一步拖慢该过程。
...
请将旧的 distfile 放到一边，下载新的文件，解压后使用 [diff(1)](https://man.freebsd.org/cgi/man.cgi?query=diff&sektion=1&format=html) 比较内容。如果确认没有可疑之处，再更新 **distinfo**。
...
FreeBSD Ports 通常期望软件遵循 POSIX 标准。有些软件和构建系统会基于特定操作系统或环境做出假设，这会在作为 Port 使用时引发问题。
...
一些简单的系统调用（例如 [gettimeofday(2)](https://man.freebsd.org/cgi/man.cgi?query=gettimeofday&sektion=2&format=html)、[getpid(2)](https://man.freebsd.org/cgi/man.cgi?query=getpid&sektion=2&format=html)）在 Linux® 上因为缓存和 vsyscall 优化而非常快，但在其他操作系统上就没有这些优势。在性能敏感的程序中，不要假设这些调用是廉价的。总的来说，应尽量避免使用系统调用。
