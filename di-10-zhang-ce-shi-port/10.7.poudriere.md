...
* 在向 FreeBSD bug 跟踪器提交补丁或向 Ports 树提交前测试 Port 构建
...
poudriere 的设置和使用也非常简单，它没有依赖关系，并且可以在任何受支持的 FreeBSD 版本上运行。本节将展示如何作为 Ports 贡献者日常工作流程的一部分安装、配置和使用 poudriere。
...
从 **poudriere.conf** 中 `FREEBSD_HOST` 指定的 HTTPS 服务器获取 `14.3-RELEASE` 版本的 `amd64`，创建 ZFS 文件系统 `tank/poudriere/jails/143Ramd64`，将其挂载到 **/poudriere/jails/143Ramd64**，并将 `14.3-RELEASE` 的 tarballs 提取到该文件系统中。

```sh
# poudriere jail -c -j 13i386 -v stable/13 -a i386 -m git+https
```

创建 `tank/poudriere/jails/13i386`，将其挂载到 **/poudriere/jails/13i386**，然后从 **poudriere.conf** 中指定的 `GIT_HOST` 或默认的 `git.freebsd.org` 获取 `FreeBSD-13-STABLE` 的 Git 分支的最新代码，检出到 **/poudriere/jails/13i386/usr/src**，然后完成 `buildworld` 并将其安装到 **/poudriere/jails/13i386** 中。

>**注意**
>
> 尽管可以在较旧的版本上构建较新的 FreeBSD 版本，但大多数情况下它无法运行。例如，如果需要 `stable/14` jail，则主机也必须运行 `stable/14`，仅运行 `14.3-RELEASE` 不够。

>**注意**
>
> 要为 `16.0-CURRENT` 创建 poudriere jail：
>
>```sh
># poudriere jail -c -j 16amd64 -v main -a amd64 -m git+https
>```
>
>要运行 `16.0-CURRENT` 的 poudriere jail，主机必须运行 `16.0-CURRENT`。通常情况下，较新的内核可以构建并运行较旧的 jails。例如，`16.0-CURRENT` 内核可以构建并运行 `14.3-STABLE`，前提是已编译 `COMPAT_FREEBSD14` 内核选项（默认启用，`16.0-CURRENT` 的 GENERIC 内核配置中已启用）。
...
该命令会将指定的 jail 更新到最新版本。对于 FreeBSD 发布版本，可以使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 更新到最新的补丁级别。对于从源代码构建的 FreeBSD 版本，可以更新到分支中的最新 git 修订版。
...
对于 Port 贡献者来说，处理本地修改的最佳方法是使用 [Git](https://docs.freebsd.org/en/books/handbook/mirrors/#git)。与创建 jail 类似，可以使用不同的方法来创建 ports tree。为了测试本地修改和开发 Port，最好通过 git 检出树（如上所述）来添加额外的 ports tree。
...
## 10.7.7. 保持 poudriere Ports Trees 更新
...
>**注意**
>
>没有指定更新方式的 ports trees，参见 [使用手动管理的 Ports 与 poudriere](https://docs.freebsd.org/en/books/porters-handbook/testing/#testing-poudriere-ports-tree-manual)，无法像这样更新，必须由维护者手动更新。
...
例如，本地修改位于 **/work/ports/www/firefox** 的 [www/firefox](https://cgit.freebsd.org/ports/tree/www/firefox/) Port，可以在之前创建的 14.3-RELEASE jail 中进行测试：
...
每个 Port 的完整构建过程会记录在 **/poudriere/data/logs/bulk/143Ri386-development/build-time/logs** 中。

目录名 `143Ri386-development` 是根据 `-j` 和 `-p` 参数生成的。为了方便起见，还维护了一个符号链接 **/poudriere/data/logs/bulk/143Ri386-development/latest**，该链接指向最新的 *build-time* 目录。该目录中还有一个 **index.html**，可以通过网页浏览器观察构建过程。
...
构建完成后，无论是否成功，都会提供一个 shell 进入 jail。可以使用该 shell 进一步调查。poudriere 可以通过 `-I` 选项在构建完成后保持 jail 运行。poudriere 会显示不再需要 jail 时运行的命令。此时可以使用 [jexec(8)](https://man.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&format=html) 进入 jail：
...
这会在构建 Port 之前显示 Port 配置对话框。`-o` 后面以 `_category_/_portname_` 格式给出的 Port 将使用指定的选项，所有依赖项将使用默认选项。可以使用集合测试带有非默认选项的依赖 Port，参见 [使用集合](https://docs.freebsd.org/en/books/porters-handbook/testing/#testing-poudriere-sets)。

>**技巧**
>
>当在构建过程中根据所选选项修改 **pkg-plist** 时，建议进行两次测试运行：一次选中所有选项，*另一次* 不选任何选项。
...
为了使用 sets，poudriere 需要一个类似于 `PORT_DBDIR`（默认为 **/var/db/ports**）的现有目录结构，该结构位于其配置目录中。然后，该目录会通过 [nullfs(5)](https://man.freebsd.org/cgi/man.cgi?query=nullfs&sektion=5&format=html) 挂载到构建 Port 及其依赖项的 jails 中。通常，可以通过递归复制现有的 `PORT_DBDIR` 到 **/usr/local/etc/poudriere.d/jailname-portname-setname-options** 来获得合适的起始点。详情请见 [poudriere(8)](https://man.freebsd.org/cgi/man.cgi?query=poudriere&sektion=8&format=html)。例如，要在名为 `devset` 的特定 set 中测试 [www/firefox](https://cgit.freebsd.org/ports/tree/www/firefox/)，只需在 `testport` 命令中添加 `-z devset` 参数：
...
与 sets 不同，所有找到的文件将 *按顺序* 追加到构建 jails 中的一个 **make.conf** 文件中。因此，可以在 **/usr/local/etc/poudriere.d/make.conf** 中设置影响所有构建的通用 make 变量。特定 jails 或 sets 的特殊变量可以在专用的 **make.conf** 文件中进行设置，例如 **/usr/local/etc/poudriere.d/143Ramd64-development-devset-make.conf**。
