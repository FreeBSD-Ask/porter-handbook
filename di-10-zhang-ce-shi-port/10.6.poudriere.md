...
> **注意**
>
> 没有方法的 Ports 树， 见 [Using Manually Managed Ports Trees with Poudriere](https://docs.freebsd.org/en/books/porters-handbook/testing/#testing-poudriere-ports-tree-manual)， 不能像这样更新。它们必须由 port 使用人手动更新。
...
在构建完成后，无论是否成功， jail 内都会提供一个 shell。这个 shell 是用来进一步调查的。可以告诉 Poudriere ，在构建完成后，用 -I 来保持监狱的运行。Poudriere 会在不再需要该 jail 时显示要运行的命令。然后就可以用 [jexec(8)](https://www.freebsd.org/cgi/man.cgi?query=jexec\&sektion=8\&format=html) 进入它。
