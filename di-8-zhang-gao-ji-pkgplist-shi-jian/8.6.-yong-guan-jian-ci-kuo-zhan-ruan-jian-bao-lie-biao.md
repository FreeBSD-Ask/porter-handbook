...
此关键字执行三项操作：它将第一个作为参数传递给 `@sample` 的 *filename* 添加到打包列表中；它将指令添加到 `post-install` 脚本中，以便在实际配置文件不存在时，将示例文件复制到实际配置文件中；它将指令添加到 `post-deinstall` 脚本中，在配置文件未被修改的情况下删除该配置文件。
