...
此关键字执行三项操作。首先，将作为参数传递的第一个文件（示例文件）添加到 plist。然后，在安装时，如果找不到实际文件，则将示例文件复制到实际文件。最后，在卸载时，如果实际文件没有被修改，则删除该文件。有关更多信息，请参见 [Configuration Files](https://docs.freebsd.org/en/books/porters-handbook/plist/#plist-config)。
