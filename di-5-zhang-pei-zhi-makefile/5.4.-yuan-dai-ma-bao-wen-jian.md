...
`DISTNAME` 是软件作者所调用的端口的名称。`DISTNAME` 默认为  `${PORTNAME}-${DISTVERSIONPREFIX}${DISTVERSION}${DISTVERSIONSUFFIX}`， 如果没有设置， `DISTVERSION` 默认为  `${PORTVERSION}`， 因此只有在必要时才能覆盖 `DISTNAME`。`DISTNAME` 只在两个地方使用。首先， 发行文件列表 (`DISTFILES`) 默认为 `${DISTNAME}${EXTRACT_SUFX}`。第二， 分发文件会被期望提取到名为 `WRKSRC` 的子目录中， 该目录默认为 **work/${DISTNAME}**。
...
在 `MASTER_SITES` 中记录指向原始 tarball 的 FTP/HTTP-URL 的目录部分。不要忘记尾部的斜线 (/)!
...
> > 注意
> >
> > `GL_SITE` 的协议、端口和 webroot 都可以在同一个变量中修改。
...
> 注意
>
> 到那时，这个 tarball 已经和普通的源代码一起被解压了，所以如果是普通的压缩 tarball，就没有必要明确地解压它。如果手动解压，要特别注意不要覆盖该目录中已经存在的东西。另外，不要忘了在预清洁目标中添加一个删除复制的补丁的命令。
...
好了， 前面的例子没有反映出新端口的需求？在这一节中， 我们将详细解释细粒度的获取机制 `MASTER_SITES:n` 如何工作， 以及如何使用它。
...
### 5.4.9.3. 对端口来说，什么会发生变化？什么没有？

Ⅰ. 所有当前的端口都保持不变。`MASTER_SITES:n` 功能代码只有在有元素后缀为 `:n` 的情况下才会根据上述语法规则被激活， 特别是如第 7 项所示。

Ⅱ. 端口目标保持不变： `checksum`, `makesum`, `patch`, `configure`, `build`, 等等。除了 `do-fetch`、`fetch-list`、`master-sites` 和 `patch-sites` 这些明显的例外。
