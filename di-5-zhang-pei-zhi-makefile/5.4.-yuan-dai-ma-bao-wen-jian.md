...
**Makefile** 的第二部分描述了为构建 Port 而必须下载的文件，以及这些文件的下载位置。
...
>**注意**
>
>一些 `MASTER_SITE_*` 名称较长，为了便于使用，已定义了简写形式。

**表 3. `MASTER_SITE_*` 宏的简写**
...
| `PYPI`                    | `source/${DISTNAME:C/(.).*/\1/}/${DISTNAME:C/(.*)-[0-9].*/\1/}`                  |
| `CPAN`                    | `${PORTNAME:C/-.*//}`                                                            |
...
`20260626` 是 `GH_TAGNAME` 中引用的提交的日期，而不是 **Makefile** 编辑的日期，也不是提交的日期。
...
这创建了一个随时间增长的版本控制方案，并且仍然处于版本 `0` 之前（参见 [使用 pkg-version(8) 比较版本的章节](https://docs.freebsd.org/en/books/porters-handbook/makefiles/#makefile-versions-ex-pkg-version)）：
...
这创建了一个随着时间推移（实际上是随着提交增加）的版本控制方案，并且与创建 `0.7.4` 版本不会冲突。（参见 [使用 pkg-version(8) 比较版本的章节](https://docs.freebsd.org/en/books/porters-handbook/makefiles/#makefile-versions-ex-pkg-version)）：
...
与 GitHub 类似，如果分发文件来自 [gitlab.com](https://gitlab.com/) 或托管 GitLab 软件时，可以使用以下变量，并且可能需要设置它们。
...
在尝试为来自 gitlab.com 上的 accounts-sso 用户的 libsignon-glib 版本 `1.14` 创建 Port 时，位于 [https://gitlab.com/accounts-sso/libsignon-glib/](https://gitlab.com/accounts-sso/libsignon-glib/)，**Makefile** 的内容如下，用于获取分发文件：
...
可以将多个值添加到 `GL_SITE`、`GL_ACCOUNT`、`GL_PROJECT` 和 `GL_COMMIT` 中。每个不同的值分配一个组，默认值参见 [`USE_GITLAB` 描述](https://docs.freebsd.org/en/books/porters-handbook/makefiles/#makefile-master_sites-gitlab-description)。
...
试想一下，`DISTFILES` 中有两个文件，`MASTER_SITES` 中有 20 个站点，这些站点慢得要命，其中 **beta** 文件在所有站点上都有，而 **alpha** 文件只能在第 20 个站点找到。如果维护者事先知道这个情况，却还要检查所有站点，岂不是浪费大量时间？这可不是美好周末的好开始！
...
多个分发文件可以具有相同的组。继续之前的示例，假设有一个第三个分发文件 **source3.tar.gz**，它从 `www.example.com` 下载。此时，**Makefile** 应写成如下：
...
1. 元素可以附加 `:n` 后缀，其中 *n* 是 `[^:,]\+`，也就是说，*n* 可以是任何字母数字字符串，但我们目前将其限制为 `[a-zA-Z_][0-9a-zA-Z_]+`。
...
不要让 Port 污染 **/usr/ports/distfiles**。如果 Port 需要获取大量文件，或者包含一个可能与其他 Ports 冲突的文件（例如，**Makefile**），请将 `DIST_SUBDIR` 设置为 Port 的名称（`${PORTNAME}` 或 `${PKGNAMEPREFIX}${PORTNAME}` 都可以）。这将把 `DISTDIR` 从默认的 **/usr/ports/distfiles** 改为 **/usr/ports/distfiles/\${DIST_SUBDIR}**，从而将该 Port 所需的所有文件放入该子目录中。
