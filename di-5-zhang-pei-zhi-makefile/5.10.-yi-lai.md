...
许多端口依赖于其他端口。这是包括 FreeBSD 在内的大多数类 Unix 操作系统的一个非常方便的特性。多个 port 可以共享一个共同的依赖关系， 而不是把这个依赖关系与每个需要它的 port 或软件包捆绑在一起。有七个变量可以用来确保所有需要的位子都会出现在用户的机器上。还有一些预先支持的依赖性变量用于常见的情况， 另外还有一些变量用于控制依赖性的行为。
...
将检查是否有任何版本的共享 jpeg 库，如果没有找到，将进入 ports 树的 **graphics/jpeg** 子目录来构建和安装它。
...
将检查文件或目录 **/usr/local/news/bin/innd** 是否存在， 如果没有找到， 则从 ports 树的 **news/inn** 子目录中建立并安装它。它还会查看搜索路径中是否有一个名为 `xmlcatmgr` 的可执行文件， 如果没有找到， 则会进入 **textproc/xmlcatmgr** 来构建和安装它。
...
将检查名为 unzip 的可执行文件， 并进入 ports 树的 **archivers/unzip** 子目录， 如果没有找到它， 则建立并安装它。

> 注意
>
> 这里的 "构建 "意味着从提取到编译的一切。依赖关系是在提取目标中检查的。如果目标部分与 `DEPENDS_TARGET` 相同，可以省略。
...
将检查是否有一个名为 ncftp2 的可执行文件， 如果没有找到， 将进入 ports 树中的 **net/ncftp2** 子目录来构建和安装它。

依赖关系从获取的目标中检查。如果目标部分与 `DEPENDS_TARGET` 相同，可以省略。

## 5.10.5. `EXTRACT_DEPENDS`

这个变量用于指定这个 port 需要提取的可执行文件或文件。和前一个变量一样， 它是 `path:dir`[:`target`] 元组的列表。例如，
...
依赖关系是在提取目标中检查的。如果目标部分与 `DEPENDS_TARGET` 相同，则可以省略。

> 注意
>
> 只有在提取工作尚未完成的情况下才使用这个变量（默认假设为 `tar` ），并且不能使用 `USES=tar`、`USES=lha` 或 `USES=zip`（见使用USES宏）来使其工作。
...
将进入 ports 树中的 **java/jfc** 子目录来提取它。
...
> ```
>
> (其中 X.Y 是版本号)将为每个端口添加对 gccXY 的依赖，包括 `lang/gccXY` 本身!
...
依赖关系必须明确地或通过使用 OPTIONS 框架来声明。使用其他方法如自动检测会使索引复杂化，从而给端口和包管理带来问题。
