...
|   | 版本号比较的结果并不总是显而易见的。可以使用 `pkg version`（请参见 [pkg-version(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg-version&sektion=8&format=html)）来测试两个版本号字符串的比较。例如：`% pkg version -t 0.031 0.29` `>` 输出表示版本 `0.031` 被认为大于版本 `0.29`，这一点可能对 Ports 维护者来说并不显而易见。 |
| - | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
