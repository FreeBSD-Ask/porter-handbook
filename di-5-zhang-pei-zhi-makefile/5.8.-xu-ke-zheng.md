...
| `BSD`          | BSD 许可证通用版本（弃用）                                | `FSF OSI COPYFREE` | （默认）                                 |
...
| `CC-BY-SA-3.0` | Creative Commons Attribution Share Alike 3.0                   |                        | （默认）                    |
| `CC-BY-SA-4.0` | Creative Commons Attribution Share Alike 4.0                   |                        | （默认）                    |
| `CC0-1.0`      | Creative Commons Zero v1.0 Universal                            | `FSF GPL COPYFREE`     | （默认）                    |
...
`dist-mirror`
允许再分发分发文件。分发文件将添加到 FreeBSD `MASTER_SITE_BACKUP` CDN。

`no-dist-mirror`
禁止再分发分发文件。这等同于设置 [`RESTRICTED`](https://docs.freebsd.org/en/books/porters-handbook/special/#porting-restrictions-restricted)。分发文件将 *不会* 添加到 FreeBSD `MASTER_SITE_BACKUP` CDN。

`dist-sell`
允许出售分发文件。分发文件将出现在安装映像中。

`no-dist-sell`
禁止出售分发文件。这等同于设置 [`NO_CDROM`](https://docs.freebsd.org/en/books/porters-handbook/special/#porting-restrictions-no_cdrom)。
...
`auto-accept`
默认接受许可证。除非用户定义了 `LICENSES_ASK`，否则不会显示接受许可证的提示。除非许可证声明用户必须接受其条款，否则请使用此选项。
...
>**警告**
>
> 某些缺失的权限会导致 Port（以及所有依赖于它的 Port）无法供软件包用户使用：
>
...
当 GPLv2 和 UNKNOWN 许可证的权限混合时，Port 最终会包含 `dist-mirror dist-sell pkg-mirror pkg-sell auto-accept dist-mirror no-dist-sell pkg-mirror no-pkg-sell auto-accept`。`no-*permission*` 会取消 *permission*。最终的权限列表是 *dist-mirror pkg-mirror auto-accept*。分发文件和软件包将不会出现在安装映像中。
...
包含许可证文本的文件的完整路径，通常是 **${WRKSRC}/some/file**。如果文件不在分发文件中，且其内容太长，无法放在 [`LICENSE_TEXT`](https://docs.freebsd.org/en/books/porters-handbook/makefiles/#licenses-license_text) 中，请将其放入 **${FILESDIR}** 中的新文件。
...
当一个 Port 声明“此软件可以根据 GNU 通用公共许可证或艺术许可证进行分发”时，表示可以使用其中任何一个许可证。使用如下设置：
...
当 Port 的一部分使用一个许可证，而另一部分使用不同的许可证时，使用 `multi`：
