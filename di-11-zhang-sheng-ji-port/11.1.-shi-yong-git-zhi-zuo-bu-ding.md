...
>**重要**
>
>如果升级是出于安全考虑或当前提交的 Port 存在严重缺陷，请通知 Ports 管理团队 <[portmgr@FreeBSD.org](mailto:portmgr@FreeBSD.org)>，以请求立即重新构建和重新分发该 Port 的包。否则，毫不知情的 `pkg` 用户将在几周内继续通过 `pkg install` 安装旧版本。
