...
>**重要**
>
> 如果升级是出于安全考虑， 或当前提交的 port 中存在严重的错误， 请通知 Ports Management Team <portmgr@FreeBSD.org> 以要求立即重建和重新发布该 port 的软件包。否则， 未经检查的 `pkg` 用户将继续通过 `pkg install` 来安装旧版本数周。
...
>**注意**
>
>如果有文件被添加、移动或删除，请使用包含 [git(1)](https://www.freebsd.org/cgi/man.cgi?query=git&sektion=1&format=html) 的 `add`、`mv` 和 `rm` 命令。
