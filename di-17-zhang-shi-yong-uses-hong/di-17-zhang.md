...
默认情况下，执行源代码外构建，使 `WRKSRC` 中的源代码不受构建产物影响。使用 `insource` 参数时，将执行源代码内构建。该参数应仅作为例外，在常规的源代码外构建无法工作时使用。
...
1. 通过使用 NVD 的 [CPE 搜索引擎](https://web.nvd.nist.gov/view/cpe/search) 或在 [官方 CPE 字典](https://nvd.nist.gov/feeds/json/cpe/2.0/nvdcpe-2.0.tar.gz) 中搜索软件产品的官方 CPE 条目（警告，此文件为非常大的 JSON 文件）。*切勿自行编造 CPE 数据。*
...
设置虚拟显示环境。如果环境变量 `DISPLAY` 未设置，则会将 Xvfb 添加为构建依赖，并将 `CONFIGURE_ENV` 扩展为当前运行的 Xvfb 实例的端口号。*ARGS* 参数默认值为 `install`，控制何时启动和停止虚拟显示。
...
添加对 [audio/ebur128](https://cgit.freebsd.org/ports/tree/audio/ebur128/) 的依赖。它允许通过使用 **make.conf** 中的 `DEFAULT_VERSIONS` 来透明地依赖 `rust` 或 `legacy` 版本。例如，要使用传统版本，可以在 **make.conf** 中设置 `DEFAULT_VERSIONS+=ebur128=legacy`。
...
为易于移植且以二进制形式分发的 Electron 应用程序提供支持。根据使用的参数，添加对 [devel/electron37](https://cgit.freebsd.org/ports/tree/devel/electron37/)，[devel/electron38](https://cgit.freebsd.org/ports/tree/devel/electron38/)，或 [devel/electron39](https://cgit.freebsd.org/ports/tree/devel/electron39/) 的构建和运行时依赖。
...
有关更多信息，请参阅 [使用 GNOME](https://docs.freebsd.org/en/books/porters-handbook/special/#using-gnome)。

## 17.47. `go`

>**重要**
>
> 不应为 Go 库创建 Ports，更多信息请参阅 [Go 库](https://docs.freebsd.org/en/books/porters-handbook/special/#go-libs)。


可能的参数：（无），`N.NN`，`N.NN+`，`N.NN-devel`，`modules`，`no_targets`，`run`

设置用于构建 Go 软件的默认值和目标。添加对 Go 编译器 Port 的构建依赖，Port 维护者可以设置所需的版本。如果使用 `X.Y+` 格式，则将使用 Go `X.Y`，除非默认的 Go 版本高于 `X.Y`，在这种情况下将使用默认版本。默认情况下，构建是在 GOPATH 模式下执行的。如果 Go 软件使用模块，可以通过 `modules` 参数启用模块感知模式。`no_targets` 将设置构建环境（如 `GO_ENV`、`GO_BUILDFLAGS`），但跳过创建提取和构建目标。`run` 还将添加对 Go 编译器 Port 的运行时依赖。
...
`GO_LDFLAGS`：要传递给 `go build` 命令的额外 LDFLAGS 变量，无需更改 `GO_BUILDFLAGS`。

`GO_TESTFLAGS`：要传递给 `go test` 的额外构建参数。

有关使用示例，请参阅 [构建 Go 应用程序](https://docs.freebsd.org/en/books/porters-handbook/special/#using-go)。
...
可能的参数：（无），`base`（默认），`heimdal`，`mit`，`mit-devel`，`flags`，`bootstrap`

处理 GSS-API 消费者所需的依赖项。仅提供 Kerberos 机制的库。默认情况下，或设置为 `base`，使用基本系统中的 GSS-API 库。也可以设置为 `heimdal` 来使用 [security/heimdal](https://cgit.freebsd.org/ports/tree/security/heimdal/)，或设置为 `mit` 来使用 [security/krb5](https://cgit.freebsd.org/ports/tree/security/krb5/)，或设置为 `mit-devel` 来使用 [security/krb5-devel](https://cgit.freebsd.org/ports/tree/security/krb5-devel/)。
...
`flags` 选项可以与 `base`、`heimdal`、`mit` 或 `mit-devel` 一起使用，以自动将 `GSSAPICPPFLAGS`、`GSSAPILDFLAGS` 和 `GSSAPILIBS` 添加到 `CFLAGS`、`LDFLAGS` 和 `LDADD` 中。例如，使用 `base,flags`。
...
使用 Guile 的应用程序通常只构建单一版本的 Guile。然而，扩展或库模块应使用 `flavors` 选项，以使用多个 flavors 构建。
...
`JAVA_VERSION`：Port 适用的 Java 版本列表，空格分隔。可选的 `+` 可以用来指定版本范围。（允许的值：`8[+]`，`11[+]`，`17[+]`，`18[+]`，`19[+]`，`21[+]`，`22[+]`，`23[+]`，`24[+]`，`25[+]`）
...
可能的参数：（无），`gtk2`（默认），`qt5`，`qt6`，`flavors`
...
使用 Lua 的应用程序通常仅为单个 Lua 版本构建。然而，旨在通过 Lua 代码加载的库模块应使用 `module` 选项以使用多个 flavors 构建。
...
如果指定了 `muon`，则添加对 [devel/muon](https://cgit.freebsd.org/ports/tree/devel/muon/) 的构建依赖。
...
可能的参数：（无），`build`，`env`，`run`，`current`，`lts`，`20`，`22`，`24`，`25`，`26`

使用 nodejs。添加对 [www/node\*](https://cgit.freebsd.org/ports/tree/www/node*/) 的依赖。如果指定了受支持的版本，则必须同时指定 `run` 和/或 `build`。`env` 参数设置环境变量 `NODEJS_VERSION` 和 `NODEJS_SUFFIX`。当前 `lts` 是 24 的别名，`current` 是 26 的别名。
...
`OCAML_LDLIBS`：`PREFIX` 下将自动添加到或从 **ld.conf** 中删除的目录。默认值：`${OCAML_SITELIBDIR}/${PORTNAME}`
...
为 Xfce 相关 Ports 提供支持。有关详细信息，请参阅 [使用 Xfce](https://docs.freebsd.org/en/books/porters-handbook/special/#using-xfce)。
