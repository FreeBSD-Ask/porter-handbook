...
这不仅会增加端口库的大小，还会使问题的根本原因变得难以查找，以及确切做了哪些更改。
...
补丁被保存在名为 [.filename]#patch-*# 的文件中，其中 * 表示被修补的文件的路径名，
例如 [.filename]#patch-Imakefile# 或 [.filename]#patch-src-config.h#。
文件名不以 [.filename]#patch-# 开头的补丁不会自动应用。
...
特别地，当端口使用 `Imake` 或 GNU `configure` 时，两个备份文件之间的差异，比如 [.filename]##Makefile##，是不必要的，需要删除。
如果需要编辑 [.filename]#configure.in# 并运行 `autoconf` 重新生成 `configure`，请不要取 `configure` 的差异（它通常会增长到几千行！）。
而是定义 `USES=autoreconf` 并取 [.filename]#configure.in# 的差异。

## 4.4.3. 简单的自动替换

可以直接从端口的 [.filename]#Makefile# 使用 man:sed[1] 的就地模式进行简单的替换。
这在更改使用变量的值时非常有用：
...
要快速将所有文件从 CR/LF 转换为 LF，将以下条目添加到端口的 [.filename]#Makefile# 中：
...
某些端口可能需要仅在特定的 FreeBSD 版本或特定选项启用或禁用时才应用的补丁。
有条件的补丁通过将补丁文件的完整路径放入 `EXTRA_PATCHES` 来指定。
有条件的补丁文件名通常以 [.filename]#extra-# 开头，尽管这不是必需的。
然而，它们的文件名 _不能_ 以 [.filename]#patch-# 开头。
...
#### 可选地应用补丁的示例[[slow-patch-extra-ex2]]
...
有时，某个功能需要许多补丁，此时可以将 `EXTRA_PATCHES` 指向一个目录，框架将自动应用其中所有文件名为 [.filename]#patch-*# 的补丁。

在 [.filename]#${PATCHDIR}# 中创建一个子目录，并将补丁文件移到其中。
例如：
...
然后将下面内容添加到 [.filename]#Makefile# 中：
...
框架将使用该目录中所有文件名为 [.filename]#patch-*# 的文件。
