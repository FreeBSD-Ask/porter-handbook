...
首先，当用户在端口目录中首次键入 `make` 命令时，会发生以下事件序列。
...
> 2. 执行 `extract` 目标。它会在 `DISTDIR` 中查找端口的分发文件（通常是压缩的 tarball），并将其解压缩到由 `WRKDIR`（默认为 **work**）指定的临时子目录中。
...
> 5. 执行 `build` 目标。这会进入端口的私有工作目录（**WRKSRC**）并进行构建。
> 6. 执行 `stage` 目标。这会将最终构建的文件集放入临时目录（**STAGEDIR**，详见交叉引用：特殊[staging,Staging]）。该目录的层次结构与将要安装软件包的系统相同。
> 7. 执行 `package` 目标。这将使用在 `stage` 目标期间创建的临时目录中的文件以及端口的 **pkg-plist** 来创建一个软件包。
...
建议在操作足够简单的情况下使用 **Makefile** 目标，因为这将更容易让其他人理解端口需要什么样的非默认操作。

默认操作由 **bsd.port.mk** 中的 `do-_something_` 目标执行。
例如，提取端口的命令位于 `do-extract` 目标中。
...
现在，了解用户键入 `make install` 时发生的情况，让我们继续按照推荐步骤来创建完美的端口。
