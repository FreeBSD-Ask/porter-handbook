...
还有一些关于 **pkg-\*** 文件的技巧尚未提及，它们有时会派上用场。

为了在安装包时显示一条消息，可以将消息放在 **pkg-message** 文件中。此功能通常用于显示在 `pkg install` 或 `pkg upgrade` 之后需要采取的额外安装步骤。
...
>* 请勿用空格或符号行（如 `----------`、`**********` 或 `==========`）包围消息。将格式设置交给 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html)。
...
* `maximum_version`
  仅在 `type` 为 `upgrade` 时使用。当从严格低于指定版本的版本升级时显示。

* `minimum_version`
  仅在 `type` 为 `upgrade` 时使用。当从严格高于指定版本的版本升级时显示。
...
>**重要**
>
>为了与非 UCL 格式的 **pkg-message** 文件保持兼容，UCL **pkg-message** 的第一行 *必须是* 单独的 `[`，最后一行 *必须是* 单独的 `]`。
...
>**重要**
>
>当显示升级时的消息时，限制向用户显示的时机非常重要。大多数情况下，通过使用 `maximum_version` 将其用途限制在从某个特定版本之前的版本升级时，需要执行特定操作的情况下显示。
